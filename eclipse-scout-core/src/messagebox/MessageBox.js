...
export default class MessageBox extends Widget {
  ...
  _createMessageBoxButton(text, boxButtons, option) {
    if (!text) {
      return null;
    }
    let button = boxButtons.addButton({text: text});
  ...
  _render() {
    ...
    this.abortButton = null; // button to be executed when abort() is called, e.g. when ESCAPE is pressed
    this.buttons = [];
    this.yesButton = this._createMessageBoxButton(this.yesButtonText, boxButtons, MessageBox.Buttons.YES);
    this.noButton = this._createMessageBoxButton(this.noButtonText, boxButtons, MessageBox.Buttons.NO);
    this.cancelButton = this._createMessageBoxButton(this.cancelButtonText, boxButtons, MessageBox.Buttons.CANCEL);
